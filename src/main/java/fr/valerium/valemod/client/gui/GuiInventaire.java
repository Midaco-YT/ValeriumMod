...
import fr.valerium.valemod.client.gui.buttons.ButtonCustom;
import fr.valerium.valemod.client.gui.buttons.GuiButtonJob;
...
public class GuiInventaire extends InventoryEffectRenderer
{
    ...
    public GuiInventaire(Container thePlayer) {
    	...
    	public GuiInventaire(EntityPlayerMP thePlayer) {
    	super(null);
    	}
    /**
    ...
    public void actionPerformed(GuiButton button) {
		if (button.id == 65) {
			Minecraft.getMinecraft().displayGuiScreen(new GuiInventaire(inventorySlots));
          }
		if (button.id == 64) {
    ...
    public void initGui()
    {
    	...
    	buttonList.add(new GuiButtonJob(64, guiLefte + 180, guiTope + 40, 22, 22, ""));
    	buttonList.add(new ButtonCustom(65, guiLefte + 180, guiTope + 10, 22, 22, ""));
        if (this.mc.playerController.isInCreativeMode())
        {
            this.mc.displayGuiScreen(new GuiContainerCreative(this.mc.thePlayer));
        }
    ...
    {
        this.fontRendererObj.drawString(I18n.format("container.crafting", new Object[0]), 86, 14, 0xBCBCBC);
    }
    ...
    public void drawScreen(int p_73863_1_, int p_73863_2_, float p_73863_3_)
    {
       ...
       // drawString(fontRendererObj, "Money: " + money.getMoney(), guiLefte + 85, guiTope + 68, 0xFFFFFF);//
        
        for (int i = 0; i < buttonList.size(); i++) {
            if (buttonList.get(i) instanceof ButtonCustom) {
            	ButtonCustom btn = (ButtonCustom) buttonList.get(i);
                if (btn.func_146115_a()) { // Tells you if the button is hovered by mouse
                    String[] desc = { "Hdv" };
                    @SuppressWarnings("rawtypes")
                    List temp = Arrays.asList(desc);
                    drawHoveringText(temp, p_73863_1_, p_73863_2_, fontRendererObj);
                }
            }
        }
    }
	...
	/* public void initGui() {
		buttonList.add(new GuiButton(0, 0, 0, "TEST"));
		super.initGui();
	}*/
}