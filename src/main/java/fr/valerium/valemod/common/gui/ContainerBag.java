...
import fr.valerium.valemod.common.slot.SlotBag;
...
{
    public InventoryBag invBackpack;
    public int rows;
 
    public ContainerBag(InventoryPlayer playerInv, InventoryBag inv) {
        ...
        for (j = 0; j < this.rows; ++j) {
            for (k = 0; k < 9; ++k) {
                this.addSlotToContainer(new SlotBag(inv, k + j * 9, 8 + k * 18, 18 + j * 18));
            }
        }
    ...
    public ItemStack transferStackInSlot(EntityPlayer player, int index) {
        ...
        if (slot != null && slot.getHasStack()) {
            ...
            // shift-click
            if (itemstack.getItem() instanceof ItemBag) return null;
